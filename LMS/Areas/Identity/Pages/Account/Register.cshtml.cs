...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        {
            // Retrieve the maximum user ID from each table and generate a new unique user ID in the format 'u' followed by 7 digits
            string maxStudentId = db.Students.Any() ? db.Students.Max(s => s.UId) : "u0000000";
            string maxProfId = db.Professors.Any() ? db.Professors.Max(p => p.UId) : "u0000000";
            string maxAdminId = db.Administrators.Any() ? db.Administrators.Max(a => a.UId) : "u0000000";

            // Find the highest ID and increment it
            string largestId = new List<string> { maxStudentId, maxProfId, maxAdminId }.Max();
            int numericPart = int.Parse(largestId.Substring(1)) + 1;
            string newUniqueId = $"u{numericPart:D7}";

            // Create the appropriate user object based on the role and add it to the database
            switch (role)
            {
            case "Student":
            ...
            case "Professor":
                ...
                db.Professors.Add(professorEntity);
                break;

            case "Administrator":
                var adminEntity = new Administrator
                {
                    UId = newUniqueId,
                    FName = firstName,
                    LName = lastName,
                ...
                db.Administrators.Add(adminEntity);
                break;
